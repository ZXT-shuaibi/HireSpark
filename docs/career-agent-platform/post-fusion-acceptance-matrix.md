...
| P2 | 字体治理与导出策略 | `font-manifest.yaml` 记录字体来源/授权/加载/回退；内置 `NotoSansSC-Regular.ttf`；`ResumeRenderValidationResult` 输出 `fontStrategy`，导出记录保留模板版本、渲染引擎、traceId 和失败原因 | `ResumeRenderPipelineTest` |
