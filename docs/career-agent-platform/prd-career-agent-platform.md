...
| AI-Meeting | 题级分布式锁、stepIdempotency、轮次补偿 | 引入答题轮次幂等键、同题重复提交防重、评分失败可重试、异常轮次补偿任务 | Phase 2/3 |
| AI-Meeting | 分布式 Single-flight、Fencing Token、心跳与结果回放 | 抽象为 Career AI 调用治理层，优先覆盖评分、追问、优化、报告生成，避免重复扣费和重复产物 | Phase 3 |
...
| AI-Meeting | Skill 业务知识体系 | 将 Career 业务规则、Rubric、Prompt、状态机和排障手册沉淀为可被 Agent 消费的项目知识单元 | Phase 3 |
...
Phase 2 先实现单实例幂等和失败重试。Phase 3 再升级为多实例分布式锁、Single-flight 和补偿调度。
...
| A19 | 面试答题评分失败 | 用户提交答案后评分链路异常 | 系统保留答案，提示可重试本题评分，不自动跳到下一题 |
...
- Career Skill 业务知识体系，把 Rubric、Prompt、状态机、排障和验收规则沉淀为 Agent 可消费文档。
