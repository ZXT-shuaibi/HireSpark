...
public class JdAlignmentInterviewAgent {
    ...
    /**
     * 生成面试前的 JD 对齐摘要，作为后续计划和提问的共同上下文。
     */
    public Map<String, Object> align(ResumeVersionDO resumeVersion,
                                     JobDescriptionDO job,
                                     CareerRetrievalEnhancement enhancement) {
        String scene = CareerInterviewAgentType.JD_ALIGNMENT.scene();
        String prompt = String.format(CareerPromptTemplates.INTERVIEW_JD_ALIGNMENT,
    ...
    private String buildKey(String scene, String userId, String artifactId) {
        return String.join(":",
                StrUtil.blankToDefault(scene, "INTERVIEW_JD_ALIGNMENT"),
