...
class CareerJsonParserTest {
    ...
    @Test
    void parsesJsonWrappedInExplanatoryText() {
        Map<String, Object> result = parser.parseObject("Here is the JSON:\n{\"score\":91,\"summary\":\"ready\"}\nDone.");

        Assertions.assertEquals(91, result.get("score"));
        Assertions.assertEquals("ready", result.get("summary"));
    }
    ...
    void rejectsInvalidJsonResponse() {
        IllegalArgumentException ex = Assertions.assertThrows(
                IllegalArgumentException.class,
                () -> parser.parseObject("not json")
        );

        Assertions.assertEquals("AI response is not valid JSON", ex.getMessage());
    }
}
